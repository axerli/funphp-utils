...
use Funphp\Utils\Contracts\Jsonable;
...
class Collection implements ArrayAccess, Arrayable, Countable, IteratorAggregate, Jsonable, JsonSerializable
{
	...
	{
		return $value instanceof self ? new static($value) : new static(ArrayHelper::wrap($value));
	}
	...
	{
		return new static(ArrayHelper::crossJoin($this->items, ...array_map([$this, 'getArrayableItems'], $lists)));
	}
	...
	public function partition($key, $operator = null, $value = null): self
	{
		...
		foreach ($this->items as $_key => $item) {
			$partitions[(int) !$callback($item, $_key)][$_key] = $item;
		}
	...
	{
		if ($number === null) {
			return ArrayHelper::random($this->items);
		}
		return new static(ArrayHelper::random($this->items, $number));
	}
	...
	{
		return new static(ArrayHelper::shuffle($this->items, $seed));
	}
